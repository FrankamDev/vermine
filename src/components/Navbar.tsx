...
// };
import React, { useState, useEffect } from "react";
import { Menu, X, Compass, Activity, Sun, Moon } from "lucide-react";
import { Link } from "react-router-dom";
...
export const Navbar: React.FC = () => {
  ...
  return (
    <nav
      className={`fixed top-0 left-0 w-full z-50 transition-all duration-500 ease-out ${
        isScrolled
          ? "bg-white/95 dark:bg-slate-950/95 backdrop-blur-lg shadow-md border-b border-slate-100 dark:border-slate-900 py-3"
          : "bg-transparent py-5"
      }`}
    >
      <div className="max-w-7xl mx-auto px-6">
        <div className="flex items-center justify-between h-14">
          ...
          >
            <div className="w-10 h-10 bg-blue-950 dark:bg-lime-400 text-white dark:text-slate-950 flex items-center justify-center font-black rounded-xl shadow-lg shadow-blue-950/10 dark:shadow-lime-400/20 group-hover:scale-105 transition-all duration-300">
              <Compass className="h-5 w-5" />
            </div>
            <span className="text-xl font-black tracking-tight text-blue-950 dark:text-white uppercase">
              VERMINE <br />
              <span className="text-lime-600 dark:text-lime-400">secret</span>
            </span>
          </Link>
          ...
          <div className="hidden md:flex items-center space-x-8">
            <Link
              to="/"
              className="relative text-xs uppercase tracking-wider font-bold text-slate-700 dark:text-slate-300 hover:text-blue-950 dark:hover:text-white transition-colors duration-200 after:absolute after:bottom-[-4px] after:left-0 after:h-[2px] after:bg-lime-500 after:w-0 hover:after:w-full after:transition-all"
            >
              Accueil
            </Link>
            <Link
              to="/services"
              className="text-xs uppercase tracking-wider font-bold text-slate-600 dark:text-slate-400 hover:text-blue-950 dark:hover:text-white transition-colors duration-200"
            >
              Nos Services
            </Link>
            <Link
            ...
            <button
              onClick={toggleTheme}
              className="p-2.5 rounded-xl bg-slate-100 dark:bg-slate-900 hover:bg-slate-200 dark:hover:bg-slate-800 text-slate-600 dark:text-slate-400 hover:text-lime-600 dark:hover:text-lime-400 transition-all duration-300 active:scale-95"
              aria-label="Toggle dark mode"
            >
              {isDark ? (
          ...
          <div className="hidden md:block">
            <a
              href="tel:+237690461830"
              className="group inline-flex items-center gap-2 bg-blue-950 hover:bg-blue-900 dark:bg-lime-400 dark:hover:bg-lime-500 text-white dark:text-slate-950 font-bold px-5 py-2.5 rounded shadow-md dark:shadow-none transition-all duration-300 active:scale-[0.97]"
            >
              <Activity className="h-4 w-4 text-lime-400 dark:text-blue-950 group-hover:animate-pulse" />
          ...
          <div className="flex items-center gap-2 md:hidden">
            ...
            <button
              onClick={toggleTheme}
              className="p-2.5 rounded-xl bg-slate-100 dark:bg-slate-900 text-slate-700 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-800 transition-all active:scale-95"
              aria-label="Toggle dark mode"
            >
              {isDark ? (
                <Sun className="h-5 w-5 text-lime-500" />
            ...
            <button
              onClick={() => setIsOpen(!isOpen)}
              className="p-2.5 text-blue-950 dark:text-white transition-transform duration-300 active:scale-90"
              aria-label="Menu"
            >
              {isOpen ? (
                <X className="h-7 w-7" />
      ...
      >
        <div className="bg-white dark:bg-slate-950 border-t border-slate-100 dark:border-slate-900 px-6 py-8 space-y-5 shadow-xl">
          <Link
            to="/"
            onClick={() => setIsOpen(false)}
            className="block text-sm uppercase tracking-wider font-bold text-blue-950 dark:text-slate-200 hover:text-lime-500 transition-colors"
          >
            Accueil
          </Link>
          <Link
            to="/services"
            onClick={() => setIsOpen(false)}
            className="block text-sm uppercase tracking-wider font-bold text-slate-600 dark:text-slate-400 hover:text-blue-950 dark:hover:text-white transition-colors"
          >
            Nos Services
          </Link>
          <Link
          ...
          <a
            href="tel:+237690461830"
            onClick={() => setIsOpen(false)}
            className="flex items-center justify-center gap-3 bg-lime-400 hover:bg-lime-500 text-blue-950 font-black py-4 rounded-xl w-full shadow-lg shadow-lime-400/20 active:scale-[0.97] transition-all text-xs uppercase tracking-wider"
          >
