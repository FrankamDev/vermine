...
import { motion, AnimatePresence } from 'framer-motion';
import { 
  Mail, Phone, MapPin, ArrowRight, ShieldAlert, CheckCircle2, 
  Clock, AlertTriangle, Building, Loader2 
} from 'lucide-react';
import { Navbar } from '../components/Navbar';
...
export function Contact() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // 1. Simulation de la validation réseau locale (2 secondes)
    await new Promise(resolve => setTimeout(resolve, 2000));
    setStatus('success');

    // 2. Mappage des secteurs en texte clair pour le message
    const sectorLabels: Record<string, string> = {
      corporate: "🏢 Bureaux, Banques & Établissements Privés",
      residential: "🏠 Résidences, Villas & Propriétés individuelles",
      industrial: "🏭 Usines, Entrepôts & Stockages Agro-industriels"
    };

    // 3. Construction du message WhatsApp parfaitement formaté
    const whatsappNumber = "237690461830";
    const statusHeader = urgency === 'critical' ? "🚨 *URGENCE CRITIQUE SOS*" : "📅 *DEMANDE D'AUDIT STANDARD*";
...
📍 *Secteur d'activité :* ${sectorLabels[formData.sector] || formData.sector}

📝 *Description des symptômes / vecteurs :*
"${formData.message}"
-----------------------------------------
📱 _Formulaire transmis via l'infrastructure Vermine Secret_`;

    // 4. Encodage et redirection automatique après un court instant (laisse le temps de voir le succès)
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch max-w-6xl mx-auto">
          ...
          <div className="lg:col-span-5 flex flex-col justify-between space-y-8 bg-white dark:bg-slate-900/40 border border-slate-200/60 dark:border-slate-900 p-6 sm:p-8 relative">
            <div className="space-y-6">
              ...
              {/* PANNEAU DYNAMIQUE SELON L'URGENCE */}
              <div className="h-28 flex flex-col justify-center">
                <AnimatePresence mode="wait">
                  {urgency === 'standard' ? (
                    <motion.div
              ...
              <div className="space-y-4 pt-4 border-t border-slate-100 dark:border-slate-800/60">
                ...
                >
                  ...
                  <div>
                    <span className="text-[9px] uppercase font-bold text-slate-400 block tracking-widest">Ligne Directe Techniciens</span>
                    <span className={`text-sm font-mono font-bold ${urgency === 'critical' ? 'text-rose-500' : 'text-slate-950 dark:text-white'}`}>+237 690 46 18 30</span>
                  </div>
                </a>
                ...
                <div className="flex items-center gap-4 p-4 border border-slate-100 dark:border-slate-900 bg-stone-50/50 dark:bg-slate-950/40 text-left">
                  ...
                  <div>
                    <span className="text-[9px] uppercase font-bold text-slate-400 block tracking-widest">Secrétariat Central</span>
                    <span className="text-sm font-mono font-medium text-slate-950 dark:text-white">contact.vermine@gmail.com</span>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-4 p-4 border border-slate-100 dark:border-slate-900 bg-stone-50/50 dark:bg-slate-950/40 text-left">
                  ...
                  <div>
                    <span className="text-[9px] uppercase font-bold text-slate-400 block tracking-widest">Bureaux Généraux</span>
                    <span className="text-sm font-sans font-light text-slate-700 dark:text-slate-300">Yaoundé Ngousso, Cameroun</span>
                  </div>
                </div>
              </div>
          ...
          {/* COLONNE DROITE : FORMULAIRE PRO EXÉCUTIF */}
          <div className="lg:col-span-7 bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800/80 p-6 sm:p-8 relative shadow-xl shadow-black/[0.02]">
            
            <AnimatePresence mode="wait">
              {status === 'success' ? (
                // ÉCRAN SUCCÈS IMMERSIF AVEC LOGIQUE REDIRECTION
                <motion.div 
                  initial={{ opacity: 0, scale: 0.98 }}
                  animate={{ opacity: 1, scale: 1 }}
                  exit={{ opacity: 0 }}
                ...
                >
                  ...
                  <p className="text-xs text-slate-500 dark:text-slate-400 font-light max-w-sm mb-6 leading-relaxed">
                    Ouverture sécurisée de l'interface WhatsApp. Veuillez cliquer sur envoyer sur votre appareil pour valider votre demande auprès du répartiteur.
                  </p>
                  <button
                    onClick={() => setStatus('idle')}
                    className="px-5 py-2.5 border border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-400 text-[10px] font-bold uppercase tracking-widest hover:bg-stone-50 dark:hover:bg-slate-950 transition-colors"
                ...
                // FORMULAIRE AVEC LES CHAMPS D'ORIGINE PRÉSERVÉS
                <form onSubmit={handleSubmit} className="space-y-5 text-left h-full flex flex-col justify-between">
                  <div className="space-y-4">
                  ...
                  <div className="pt-4">
                    ...
                    >
                      {status === 'submitting' ? (
                        <>
                          <Loader2 className="w-4 h-4 animate-spin text-lime-500" />
                          <span>Traitement cryptographique des données...</span>
                        </>
                      ) : (
                        <>
                          <span>{urgency === 'critical' ? "Lancer l'alerte d'urgence" : "Soumettre la demande d'intervention"}</span>
